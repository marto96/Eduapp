...
export function StudentGradesChartWidget() {
  ...
  const { data: gradebook, isLoading: loadingGradebook } = useGradebook(activeEnrollment?.id ?? null);
  ...
  const isLoading = loadingEnrollments || (activeEnrollment !== null && loadingGradebook);
  ...
  return (
    <Card ref={cardRef}>
      <p className="text-[10px] uppercase tracking-wide text-primary">Mis notas</p>
      {isLoading ? (

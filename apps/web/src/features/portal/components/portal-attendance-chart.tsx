...
/**
 * Variante del `StudentAttendanceChartWidget` (panel del propio estudiante)
 * para el portal de familia: los registros ya vienen filtrados por hijo
 * desde `PortalView`, así que este componente solo los recibe por prop en
 * vez de buscarlos con `useMyAttendance` — mismo estilo de gráfica.
 */
export function PortalAttendanceChart({ attendance }: { attendance: AttendanceRecord[] }) {
  ...
  useGSAP(
    ...
    { scope: cardRef, dependencies: [attendance] },
  );
  ...
  return (
    <Card ref={cardRef}>
      <p className="text-[10px] uppercase tracking-wide text-primary">Asistencia</p>
      {total === 0 ? (

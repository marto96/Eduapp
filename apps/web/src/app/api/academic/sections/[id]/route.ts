...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const section = await serverApiFetch<Section>(`/academic/sections/${params.id}`, {
    method: 'PATCH',
    body: JSON.stringify(body),
  });
  if (section === null) {
    return NextResponse.json({ message: 'No se pudo editar la sección' }, { status: 400 });
  }
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const apiRes = await fetch(`${API_URL}/academic/sections/${params.id}`, {
    method: 'DELETE',
    headers: {
  ...
  if (!apiRes.ok) {
    ...
    const message = responseBody?.message ?? 'No se pudo eliminar la sección';
